...
public class KTH_DeckManager : MonoBehaviour
{
    ...
    // =========================================================
    //
    // 예전에는 맨 앞에서 LSO_TutorialDirector.PracticeCard 를 먼저 보고,
    // 잡히면 그 카드 5장으로 덱을 채운 뒤 그대로 돌아섰다.
    //
    // 그래서 덱의 내용을 정하는 주체가 둘이 됐다 — 보유 카드 목록과 튜토리얼.
    // 게다가 튜토리얼 쪽은 정적 프로퍼티를 여기서 끌어다 보는 방식이라
    // 인스펙터 어디에도 안 보였다. 전투 씬에 튜토리얼 디렉터가 켜진 채로 남아
    // 있으면 덱을 무엇으로 설정하든 조용히 실습 카드 5장이 됐고,
    // 어디서 덮였는지 찾을 단서가 화면에 없었다.
    //
    // 이제 덱은 보유 카드 목록(LSO_ItemLibraryManager) 하나만 보고 만든다.
    // 튜토리얼에서 특정 카드만 쓰게 하려면 그 씬의 Unlocked Pieces 에
    // 그 카드를 필요한 장수만큼 넣으면 된다 — 중복도 그대로 들어간다.
    // =========================================================
