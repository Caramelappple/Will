...
- 참고 이미지 형식: 왼쪽 페이지는 턴별 투자 내역, 오른쪽은 `수탈 숫자 / 탐욕 숫자 / 구분선 / 보상 세 줄` 순으로 배치. 달성한 보상 문구 위에 붉은 취소선을 왼쪽에서 오른쪽으로 긋기.
- 가운데 가로 구분선은 `DLJ_FoxKingLedger > LedgerRule`의 `DLJ_LedgerRule` 컴포넌트에서 조절. `Line Color` 색상, `Draw Duration` 그리는 시간(기본 0.55초), `Start Delay` 시작 지연(기본 0.1초). 장부 활성화 시 왼쪽부터 그리며 편집 모드에서는 완성된 선 표시. 구분선 전용 셰이더와 Renderer별 색상 적용으로 책 표지 머티리얼과 분리.
...
- 왼쪽 페이지에는 투자 효과를 합산해 `회복 +N` 또는 `다음 공격 +N`으로 표시하며 사용 자원은 표시하지 않음. 회복 투자는 다음 턴 시작부터 0.85초 동안 서서히 사라지고, 다음 공격 증가는 실제 공격에 보너스가 소비되는 순간 즉시 사라짐. TMP가 지원하는 `<alpha=#XX>`만 사용해 행별 투명도를 지정하고, 지원하지 않는 닫는 alpha 태그는 사용하지 않음.
- 투자 시 여우왕 본체가 0.55초 동안 약하게 발광. 회복 투자는 초록색, 다음 공격 증가는 빨간색이며, 한 번에 여러 투자가 일어나면 색상 펄스를 순서대로 재생.
...
2026-09-14 가운데 구분선: 최신 게임 C# 컴파일 통과. 별도 Unity 플레이 모드에서 색상 변경, 공유 머티리얼 보존, 시작 지연, 재활성, 일시정지 시간 지원, 셰이더 컴파일 등 15개 검사 통과. 실제 렌더 픽셀 비교로 진행도 0에서 숨김, 고정된 왼쪽 끝, 오른쪽 끝의 순차 증가 확인.
