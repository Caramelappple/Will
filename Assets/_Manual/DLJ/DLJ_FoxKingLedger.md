...
- 참고 이미지 형식: 왼쪽 페이지는 비우고 오른쪽에 `수탈 숫자 / 탐욕 숫자 / 구분선 / 보상 세 줄` 순으로 배치. 달성한 보상 오른쪽에 붉은 체크 표시.
...
- `DLJ_LedgerInkText`의 `Animate Ink`는 수탈·탐욕 숫자 두 오브젝트에서만 활성화. 숫자가 등장하거나 바뀌면 0.65초 동안 번진 후 마르는 효과 적용. 같은 값은 다시 재생하지 않음. 제목·보상 문구·체크·`-` 표시는 즉시 보이며 애니메이션 없음. `Spread Duration`으로 시간 조절.
- `DLJ/Ledger Ink` 셰이더와 Renderer별 MaterialPropertyBlock 사용. 공유 폰트/머티리얼을 런타임에 변경하지 않음.
- 글자·구분선·체크 메시를 종이 곡면에 맞춰 배치. 수치 기본 글자 크기 4.2, 제목 3.8, 보상 2.05; 영역에 맞춰 자동 축소.
- 프리팹의 `Milestone Rows` / `Milestone Checks`는 기존 여우왕 보상 3단계에 대응. 보상 단계를 추가하려면 표시 행과 체크도 같은 순서로 추가.
...
Unity 6000.3.6f1 별도 배치 프로젝트의 실제 플레이 모드에서 73개 검사 통과.
...
잉크 진행도 0 / 0.35 / 1 렌더링 확인. 숫자가 보이지 않는 진행도 0에서도 제목·보상·체크가 유지되는지 확인. 미리보기는 별도 기본 렌더 파이프라인 조명이며 실제 게임 씬의 URP 조명·후처리 결과와는 차이가 있음.
